angular.module('crowdy')
 ...
 '$sce', '$controller', '$http',
  function($rootScope, $scope, $routeParams, $modal, $log, $sce, $controller, $http) {
    ...
    $scope.performanceFeedback = null;
  ...
  $scope.p.loadTaskSuccess = function(data) {
      ...
      // Set up feedback if the server gave it to us
      if (data.performanceFeedback) {
          var pf = data.performanceFeedback;
          pf.scale = {
            performance: 100*(pf.worker.performance/(pf.max-pf.min)),
            rank: (100*pf.worker.rank/pf.numWorkers)
          };
          pf.percentile = Math.round(pf.scale.rank);

        $scope.performanceFeedback = pf;
    ...
    $scope.taskset.bonus = 0;
      ...
      if ($scope.bonusStructure.laterTask && $scope.taskset.meta.countUserHIT >= 1) {
   ...
   // SAVE TASK
    $scope.submitForm = function() {
        ...
        if ($scope.condition === 'training') {
            $scope.allChecked = _.every($scope.taskset.tasks, {'checked':true});
