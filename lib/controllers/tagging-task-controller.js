...
angular.module('crowdy')
 .controller("tagTaskController", ['$http', '$sce', '$httpParamSerializerJQLike', '$log', '$modal', 
        '$scope', '$rootScope', '$routeParams', function($http, $sce, $httpParamSerializerJQLike, 
        $log, $modal, $scope, $rootScope, $routeParams) {
    ...
    $scope.strikethrough = function(test){
        if (test) {
            //return { 'text-decoration': 'strike-through' };
    ...
    $http.jsonp(endpoint, {params:params})
     .success(function(data){
        ...
        if ($scope.condition === 'fast') {
            ...
            if (!data.timer) {
                $log.error("No timer info provided for fast conditions.");
                $scope.openModal({
                    show:{ok:false},
            ...
            $scope.time.limit = $rootScope.debug ? 15 : data.timer;
            ...
            $log.error("This condition is not supported.");
        }
    ...
    $scope.submitForm = function() {
        ...
        if ($scope.condition === 'fast' && $scope.phase !== 'feedback'){
