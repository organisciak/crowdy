    ...
    $http.jsonp(endpoint, {params:params})
     .success(function(data){
        ...
        if ($scope.condition === 'fast') {
            ...
            if (data.hit.itemTimeEstimate) {
                $log.warn("Ignoring time.estimate for fast condition, using timer.");
            }
            $scope.time.limit = $rootScope.debug ? 15 : data.hit.timer;
            ...
            if (data.hit.timer) {
                ...
                if (!data.hit.itemTimeEstimate) {
                    ...
                    $scope.time.estimate = Math.round(10 * data.hit.itemTimeEstimate * $scope.taskset.tasks.length)/10;
                }
            }
            //do something
        } else {
