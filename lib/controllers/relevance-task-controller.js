(function() {
    ...
    '$scope', '$rootScope', '$routeParams', '$modal', function($controller,
    $http, $log, $scope, $rootScope, $routeParams, $modal) {
    ...
    $scope.p.taskTypePostLoadPrep = function(data) {
      ...
      // Set up training Modal intervention if the condition calls for it
      if (data.condition === 'training2') {
        ...
        resolve:{
            opts: function(){
                return { training: data.taskset.facet.meta };
                            }
                }
        });    
