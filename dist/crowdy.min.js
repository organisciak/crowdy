...
!function() {
    ...
    b.forms = {}, b.time = {
        estimate: null
    }, b.p = {}, b.condition = c.condition, b.preview = "ASSIGNMENT_ID_NOT_AVAILABLE" === c.assignmentId, 
    ...
    b.p.conditionalPostLoadPrep || (b.p.conditionalPostLoadPrep = function() {
        ...
        b.taskset.bonus = 0, b.nextItemBonus = 0, b.p.conditionalPostLoadPrep(c), void (b.p.taskTypePostLoadPrep || function() {})(c));
    ...
    angular.module("crowdy").controller("tagTaskController", [ "$controller", "$http", "$log", "$scope", "$rootScope", "$routeParams", function(a, b, c, d, e, f) {
        d.instructions = {
            ...
            $scope: d
        }), d.include = "/templates/tagging/" + f.condition + "-index.html", d.type = "tagging";
        var g, h = {
        ...
        g = f.debugHIT ? "data/debug-" + d.condition + "-image-hit.json" : d.BACKEND_SERVER + "/hit", 
        b.jsonp(g, {
            params: h
    ...
    angular.module("crowdy").controller("relevanceTaskController", [ "$controller", "$http", "$log", "$scope", "$rootScope", "$routeParams", function(a, b, c, d, e, f) {
        d.instructions = {
            title: "Judge if an image is relevance to a web search",
            meta: null
        }, a("taskController", {
            $scope: d
        }), d.include = "/templates/relevance/" + f.condition + "-index.html", d.judgeOptions = [ "Very Relevant", "Somewhat Relevant", "Not Relevant" ], 
        d.type = "relevance";
        var g, h = {
            callback: "JSON_CALLBACK",
            user: d.preview ? "PREVIEWUSER" : f.workerId,
        ...
        d.p.taskTypePostLoadPrep = function(a) {
            d.instructions.title = "Judge if an image is relevance to a web search for '" + a.taskset.facet.meta.query + "'", 
            d.instructions.meta = a.taskset.facet.meta;
        }, b.jsonp(g, {
            params: h
        }).success(d.p.loadTaskSuccess).error(d.p.loadTaskError);
    } ]);
}(), function() {
    angular.module("crowdy").controller("relevanceTaskItemController", [ "$http", "$log", "$scope", function(a, b, c) {
        var d;
        c.tooltip = function(a) {
            return "<em>Title: </em><strong>" + a.meta.title + "</strong><br/><em>Description: </em>" + a.meta.description;
        }, c.timeStart = function(a) {
            ...
            a.timing && (d = (new Date() - a.timing) / 1e3, a.timeSpent += d, delete a.timing);
        };
    } ]);
}(), function() {
