...
!function() {
    ...
    angular.module("crowdy").controller("tagTaskController", [ "$http", "$sce", "$httpParamSerializerJQLike", "$log", "$modal", "$scope", "$rootScope", "$routeParams", function(a, b, c, d, e, f, g, h) {
        ...
        m = h.debugHIT ? "data/debug-image-hit.json" : i + "/hit", f.completedItems = function() {
            var a = (f.taskset.tasks || []).length - (f.forms.turkForm.$error.pattern || []).length - (f.forms.turkForm.$error.required || []).length;
            return a;
        }, f.strikethrough = function(a) {
            return a ? {
                "text-decoration": "line-through"
            } : void 0;
        };
        var o = function() {
            var a = f.completedItems();
            if (!a && !f.bonusStructure.items) return 0;
            var b = f.bonusStructure.items;
            a < b.length ? f.nextItemBonus = b[a] : f.nextItemBonus = _.last(b);
            var c = _.reduce(_.take(b, a), _.add);
            a > b.length && (c += (a - b.length) * _.last(b)), f.taskset.bonus = _.round(c, 2);
        };
        a.jsonp(m, {
            params: n
        }).success(function(a) {
            ...
            })), f.taskset = a.taskset, f.taskset.payment = a.payment.base, f.bonusStructure = a.payment.bonus, 
            f.taskset.bonus = 0, f.nextItemBonus = 0, "fast" === f.condition) {
                if (f.stagingTasks = a.taskset.tasks, f.taskset.tasks = [ f.stagingTasks.shift() ], 
                !a.timer) return d.error("No timer info provided for fast conditions."), void f.openModal({
                    show: {
                        ok: !1
                    },
                ...
                a.itemTimeEstimate && d.warn("Ignoring time.estimate for fast condition, using timer."), 
                f.time.limit = g.debug ? 15 : a.timer, f.time.left = f.time.limit, f.time.estimate = f.time.limit;
            } else "basic" === f.condition ? a.timer && (d.warn("Ignoring timer for basic condition."), 
            a.itemTimeEstimate ? f.time.estimate = Math.round(10 * a.itemTimeEstimate * f.taskset.tasks.length) / 10 : f.time.estimate = 210) : d.error("This condition is not supported.");
        }).error(function(a) {
            g.error = !0;
            var b, c;
